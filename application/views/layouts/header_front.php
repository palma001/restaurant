<!DOCTYPE html>
<html dir="ltr" lang="en-US">
	<head>
		<meta charset="utf-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<title>
			<?php if ($settings){ echo $settings->title; } ?>
		</title>
		 <!--IE Compatibility Meta-->
	    <meta name="author" content="zytheme" />
	    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
	    <meta name="description" content="Elegant Restaurant & Cafe Html5 Template">
	    <link href="<?php echo base_url('uploads/logo/'.$settings->favicon); ?>" rel="icon">
	    <!-- Fonts
	    ============================================= -->
	    <link href="http://fonts.googleapis.com/css?family=Great+Vibes%7CKaushan+Script%7CRaleway:100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet" type="text/css">
	    <link href="<?php echo base_url('frontend_template/assets/css/external.css'); ?>" rel="stylesheet">
	    <link href="<?php echo base_url('frontend_template/assets/css/bootstrap.min.css'); ?>" rel="stylesheet">
	    <link href="<?php echo base_url('frontend_template/assets/css/style.css'); ?>" rel="stylesheet">
	    <link href="<?php echo base_url('frontend_template/assets/css/external.css'); ?>" rel="stylesheet">
	    <link href="<?php echo base_url('frontend_template/assets/css/bootstrap.min.css'); ?>" rel="stylesheet">
	    <link href="<?php echo base_url('frontend_template/assets/css/style.css'); ?>" rel="stylesheet">
	    <link href="<?php echo base_url('frontend_template/assets/css/style_front.css'); ?>" rel="stylesheet">
	    <!-- RS5.0 Main Stylesheet -->
	    <link rel="stylesheet" type="text/css" href="<?php echo base_url('frontend_template/assets/revolution/css/settings.css'); ?>">
	    <link rel="stylesheet" type="text/css" href="<?php echo base_url('frontend_template/assets/revolution/css/layers.css'); ?>">
	    <link rel="stylesheet" type="text/css" href="<?php echo base_url('frontend_template/assets/revolution/css/navigation.css'); ?>">
	    <link href="<?php echo base_url('admin_template/css/icons.min.css')?>" rel="stylesheet" type="text/css" />
	    <script src="<?php echo base_url('frontend_template/assets/js/jquery-2.2.4.min.js'); ?>"></script>
	    <script src="<?php echo base_url('social_buttons/js/functions.js'); ?>"></script>
	</head>
	<body>
